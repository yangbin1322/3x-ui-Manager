...
## Об этом форке (`3x-ui-Manager`)

`yangbin1322/3x-ui-Manager` расширяет исходный 3X-UI от «управления уже имеющимися узлами 3x-ui» до «управления целым парком серверов по SSH». В дополнение ко всему, что перечислено ниже, этот форк добавляет:

- **Серверы под управлением по SSH** — подключение Linux-хоста по SSH (пароль или ключ), с проверкой соединения, учётными данными, зашифрованными AES-256-GCM, проверкой ключа хоста и heartbeat-проверками доступности. Страница узлов разделена на вкладки **Servers** и **Panel Nodes**.
- **Удалённая установка / импорт / удаление** — установка 3x-ui на сервер (с пошаговой карточкой настройки: учётная запись, порт, путь, тип БД, режим SSL, включая сертификаты по IP или Let's Encrypt) с автоматическим созданием связанного узла панели, импорт существующей панели или удаление — по одному или пакетно.
- **Пакетные операции** — массовое добавление серверов из вставленной/загруженной таблицы, выполнение команд на множестве серверов с журналом аудита, развёртывание конфигурации одного inbound на несколько узлов, пакетное подключение/отключение клиентов между inbound-ами.
- **Распространение файлов** — загрузка файлов/каталогов по пути через SFTP и копирование пути между серверами, с общим индикатором прогресса.
- **Собственный конвейер сборки/выпуска/установки** — тег → GitHub Release (бинарники + встроенный UI), а `install.sh` / `update.sh` / `x-ui.sh` скачивают всё из этого форка, при этом `XUI_SECRET_KEY` автоматически генерируется и сохраняется при новых установках.

Полные сведения и история PR: **[docs/FORK_FEATURES.md](docs/FORK_FEATURES.md)**.
