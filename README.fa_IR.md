...
## درباره‌ی این فورک (`3x-ui-Manager`)

فورک `yangbin1322/3x-ui-Manager` نسخه‌ی اصلی 3X-UI را از «مدیریت نودهای 3x-ui که از قبل دارید» به «مدیریت یک ناوگان کامل از سرورها از طریق SSH» گسترش می‌دهد. علاوه بر همه‌ی موارد زیر، این فورک این‌ها را اضافه می‌کند:

- **سرورهای مدیریت‌شده با SSH** — اتصال یک میزبان لینوکسی از طریق SSH (رمز عبور یا کلید)، همراه با تست اتصال، اعتبارنامه‌های رمزگذاری‌شده با AES-256-GCM، بررسی کلید میزبان و ضربان‌های در دسترس‌بودن. صفحه‌ی نودها به دو زبانه‌ی **Servers** و **Panel Nodes** تقسیم می‌شود.
- **نصب / وارد کردن / حذف از راه دور** — نصب 3x-ui روی یک سرور (با کارت پیکربندی گام‌به‌گام: حساب، پورت، مسیر، نوع پایگاه‌داده، حالت SSL شامل گواهی‌های IP یا Let's Encrypt) و ایجاد خودکار یک نود پنل مرتبط، وارد کردن یک پنل موجود، یا حذف — به‌صورت تکی یا دسته‌ای.
- **عملیات دسته‌ای** — افزودن انبوه سرورها از یک جدول چسبانده/بارگذاری‌شده، اجرای دستورها روی چندین سرور با گزارش حسابرسی، استقرار پیکربندی یک inbound روی چندین نود، و پیوست/جداسازی دسته‌ای کلاینت‌ها بین inbound‌ها.
- **توزیع فایل** — بارگذاری فایل‌ها/دایرکتوری‌ها به یک مسیر از طریق SFTP و کپی یک مسیر بین سرورها، همراه با یک نوار پیشرفت کلی.
- **زنجیره‌ی خودمیزبان ساخت/انتشار/نصب** — تگ ← GitHub Release (باینری‌ها + رابط کاربری تعبیه‌شده)، و اسکریپت‌های `install.sh` / `update.sh` / `x-ui.sh` همگی از همین فورک دانلود می‌کنند، به‌همراه تولید و ماندگارسازی خودکار `XUI_SECRET_KEY` در نصب‌های تازه.

جزئیات کامل و تاریخچه‌ی PR: **[docs/FORK_FEATURES.md](docs/FORK_FEATURES.md)**.
