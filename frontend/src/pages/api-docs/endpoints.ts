...

  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        summary: 'Register several managed servers in one request. Each row is validated and created independently, so one bad row does not block the others; the response reports per-row success/failure in the input order (with the row index). A row with an empty name defaults to its address.',
      ...
      {
        method: 'POST',
        path: '/panel/api/managedServers/import',
        summary: 'Adopt a 3x-ui panel that is ALREADY installed on a managed server: read its running credentials over SSH (port/base path from `x-ui setting -show`, API token minted if absent) and derive a linked panel node WITHOUT reinstalling. The derived node defaults to https with TLS verification skipped (correct it from the Panel Nodes tab). A server that already has a linked node is rejected.',
        body: '{\n  "serverId": 3\n}',
        responseSchema: 'InstallResult',
      },
      {
        method: 'POST',
        path: '/panel/api/managedServers/uninstall',
        summary: 'Run the official 3x-ui uninstaller on a managed server over SSH. If the server has a derived panel node, that node is deleted and the link cleared so the panel and its record disappear together. The SSH access (the managed-server row) is kept so the box can be reinstalled. Long-running and synchronous.',
        body: '{\n  "serverId": 3\n}',
        responseSchema: 'UninstallResult',
      },
      {
        method: 'POST',
        path: '/panel/api/managedServers/uninstallBatch',
        summary: 'Uninstall 3x-ui from several managed servers at once. Servers run concurrently; the response carries one per-server outcome.',
        body: '{\n  "serverIds": [3, 5]\n}',
        responseSchema: 'BatchInstallResponse',
      },
      {
        method: 'GET',
        path: '/panel/api/managedServers/panelVersions',
        summary: 'List installable 3x-ui release tags (newest first) for the install version picker. Fetched from the MHSanaei/3x-ui GitHub releases and cached ~15 minutes; a transient GitHub failure serves the stale list. The caller adds a "latest" default and free-text entry, so an empty list is still usable.',
        response: '{\n  "success": true,\n  "obj": ["v2.6.0", "v2.5.5"]\n}',
      },
      {
        method: 'GET',
