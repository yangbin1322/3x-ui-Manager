...
## حول هذا الفورك (`3x-ui-Manager`)

يوسّع الفورك `yangbin1322/3x-ui-Manager` نسخة 3X-UI الأصلية من «إدارة عُقد 3x-ui الموجودة لديك» إلى «إدارة أسطول كامل من الخوادم عبر SSH». إضافةً إلى كل ما هو مذكور أدناه، يضيف هذا الفورك:

- **خوادم مُدارة عبر SSH** — ربط مضيف لينكس عبر SSH (كلمة مرور أو مفتاح)، مع اختبارات اتصال، وبيانات اعتماد مُشفَّرة بـ AES-256-GCM، والتحقق من مفتاح المضيف، ونبضات توفّر الاتصال. تنقسم صفحة العُقد إلى تبويبَي **Servers** و **Panel Nodes**.
- **التثبيت / الاستيراد / إلغاء التثبيت عن بُعد** — تثبيت 3x-ui على خادم (ببطاقة إعداد خطوة بخطوة: الحساب، المنفذ، المسار، نوع قاعدة البيانات، وضع SSL بما في ذلك شهادات IP أو Let's Encrypt) واشتقاق عُقدة لوحة مرتبطة تلقائيًا، أو استيراد لوحة موجودة، أو إلغاء التثبيت — فرديًا أو دفعةً واحدة.
- **العمليات المجمّعة** — إضافة الخوادم بالجملة من جدول مُلصَق/مرفوع، وتشغيل الأوامر على عدة خوادم مع سجل تدقيق، ونشر إعداد inbound واحد على عدة عُقد، وربط/فصل العملاء دفعةً واحدة عبر عدة inbound.
- **توزيع الملفات** — رفع الملفات/المجلدات إلى مسار عبر SFTP ونسخ مسار بين الخوادم، مع شريط تقدّم إجمالي.
- **سلسلة بناء/إصدار/تثبيت ذاتية الاستضافة** — وسم ← GitHub Release (ملفات ثنائية + واجهة مضمّنة)، وتُنزّل السكربتات `install.sh` / `update.sh` / `x-ui.sh` كلها من هذا الفورك، مع توليد `XUI_SECRET_KEY` وحفظه تلقائيًا في عمليات التثبيت الجديدة.

التفاصيل الكاملة وسجل طلبات الدمج: **[docs/FORK_FEATURES.md](docs/FORK_FEATURES.md)**.

التثبيت من هذا الفورك:
